...
extern "C" {
    // 向链接器说明在哪里找函数的定义
    // eh (exception handle) 是 llvm 异常处理模块，提供了 setjmp 和 longjmp
    // Hx@2023-04-23: cargo 1.71.0-nightly 这个版本不要设置 link_name 才能运行
    // 设置了会 segment fault
...
fn main() {
    ...
    let rc = unsafe {
        // setjmp 是入口也是出口，当调用longjmp后，将再次返回setjump这里
        setjmp(return_point)
    };
